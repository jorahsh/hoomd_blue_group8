...
class ABCTypeEpoxyDPDLJSimulation(ABCTypeEpoxySimulation):
    ...
    def setup_force_fields(self, stage):
        ...
        self.dpdlj = md.pair.dpdlj(r_cut=2.5, nlist=self.nl, kT=temperature, seed=123456)
        self.dpdlj.pair_coeff.set('A', 'A', epsilon=self.AA_interaction, sigma=1.0, gamma=self.gamma, alpha=self.AA_alpha)
        self.dpdlj.pair_coeff.set('B', 'B', epsilon=self.AA_interaction, sigma=1.0, gamma=self.gamma, alpha=self.AA_alpha)
        self.dpdlj.pair_coeff.set('C', 'C', epsilon=self.AA_interaction, sigma=1.0, gamma=self.gamma, alpha=self.AA_alpha)

        self.dpdlj.pair_coeff.set('A', 'B', epsilon=self.AB_interaction, sigma=1.0, gamma=self.gamma, alpha=self.AB_alpha)
        self.dpdlj.pair_coeff.set('A', 'C', epsilon=self.AC_interaction, sigma=1.0, gamma=self.gamma, alpha=self.AC_alpha)
        self.dpdlj.pair_coeff.set('B', 'C', epsilon=self.BC_interaction, sigma=1.0, gamma=self.gamma, alpha=self.BC_alpha)
